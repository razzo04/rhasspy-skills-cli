...
from ..main import app, create, install
...
def test_install_invalid_archive(tmp_path):
    ...
    with pytest.raises(click.exceptions.Exit) as exc:
        install(file_path, ["https://example.com"], cache=False, force=False, start_on_boot=False)
    assert os.listdir(tmp_path) == ["test.tar"]

def test_install(tmp_path, httpx_mock: HTTPXMock):
    httpx_mock.add_response(method="POST", json={"state":"success", "detail":"installed"})
    time_skill = get_test_resource("time_example")
    try:
        install(time_skill.as_posix(), ["https://example.com"], cache=False, force=False, start_on_boot=False)
